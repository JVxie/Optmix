  ...
  const handleSave = () => {
    ...
    for (const ind of indicators) {
      ...
      if (valStr === undefined || valStr === '' || valStr === null) {
        setError(`请填写 ${ind.name} 的数值`);
        return;
      }
      const val = parseFloat(valStr);
      if (isNaN(val)) {
        setError(`${ind.name} 的数值必须为有效数字`);
        return;
      }
  ...
  return (
    <div className="h-full flex flex-col">
      ...
      >
        <div className="space-y-4">
          ...
          <div className="grid grid-cols-2 gap-4">
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">货物名称 <span className="text-red-500">*</span></label>
              <input
                type="text"
            ...
            <div>
              <label className="block text-sm font-medium text-slate-700 dark:text-slate-300 mb-1">单价 (元/吨) <span className="text-red-500">*</span></label>
              <input
                type="number"
                className={inputClass}
      ...
      >
        <div className="space-y-4">
          <p className="text-slate-600 dark:text-slate-300">
            确定要删除货物 <span className="font-bold text-red-600 dark:text-red-400">“{materials.find(m => m.id === deleteId)?.name}”</span> 吗？
          </p>
      ...
      >
        <div className="space-y-4">
          <p className="text-slate-600 dark:text-slate-300">
            确定要删除选中的 <span className="font-bold text-red-600 dark:text-red-400">{selectedIds.size}</span> 个货物吗？
          </p>
          <div className="flex gap-3 justify-end">
            <button
